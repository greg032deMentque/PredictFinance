...
namespace BackPredictFinance.API.Middleware
{
    public class ExceptionMiddleware
    {
        ...
        public async Task InvokeAsync(HttpContext context, IAuthenticationService auth)
        {
            ...
            {
                // Requête annulée par le client (navigation/refresh Angular, takeUntilDestroyed) :
                // cas attendu, pas une erreur. On ne logue pas et on n'écrit pas de réponse
                // (la connexion est déjà fermée). Les annulations côté serveur (timeout réel)
                // ne passent pas ici et restent traitées en 504.
