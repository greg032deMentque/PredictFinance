...
namespace BackPredictFinance.Services.ClientFinanceServices.AnalysisV1
{
    public sealed class AnalysisRequestCompatibilityResolver : IAnalysisRequestCompatibilityResolver
    {
        ...
        private readonly PythonCliOptions _pythonCliOptions;
        ...
        public async Task<AnalysisRequest> ResolveAsync(AnalysisRunRequestViewModel request, string userId, CancellationToken ct = default)
        {
            ...
            var resolvedPatternIds = ResolvePatternIds(request.RequestedPattern);
            ...
            return new AnalysisRequest
            {
                ...
                HistoryStartDate = BuildHistoryStartDate(asOfDate)
            };
        }
        ...
        private IReadOnlyList<string> ResolvePatternIds(string? requestedPattern)
        {
            var normalized = (requestedPattern ?? string.Empty).Trim().ToUpperInvariant();
            if (!string.IsNullOrWhiteSpace(normalized))
            {
                return [_patternRegistry.ResolveRequestedPattern(normalized).PatternId];
            }
        ...
        private DateOnly BuildHistoryStartDate(DateOnly historyEndDate)
        {
            var period = (_pythonCliOptions.Period ?? string.Empty).Trim().ToLowerInvariant();
            if (string.IsNullOrWhiteSpace(period))
            {
                throw new InvalidOperationException("La periode d'analyse Python n'est pas configuree.");
            }

            if (period.EndsWith("mo", StringComparison.Ordinal))
            {
                return historyEndDate.AddMonths(-ParsePositiveInteger(period[..^2], period));
            }

            if (period.EndsWith("y", StringComparison.Ordinal))
            {
                return historyEndDate.AddYears(-ParsePositiveInteger(period[..^1], period));
            }

            if (period.EndsWith("d", StringComparison.Ordinal))
            {
                return historyEndDate.AddDays(-ParsePositiveInteger(period[..^1], period));
            }

            throw new InvalidOperationException($"La periode d'analyse '{_pythonCliOptions.Period}' n'est pas prise en charge pour calculer la fenetre historique.");
        }

        private static int ParsePositiveInteger(string rawValue, string originalPeriod)
        {
            ...
            throw new InvalidOperationException($"La periode d'analyse '{originalPeriod}' n'est pas valide.");
        }
    }
}
