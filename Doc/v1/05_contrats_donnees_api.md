...
> **État réel** : le snapshot (`AnalysisRun` + payload JSON `analysis-snapshot-history@prompt5`) persiste bien les **niveaux de risque** (`TargetPrice` / `InvalidationPrice` par ligne de pattern) et le contexte de portefeuille, ce qui suffit à l'évaluation ex post (§2.7.1). En revanche la **lecture support** (score composite, PEA, zones support/résistance) **n'est pas persistée** : elle est recalculée à la volée depuis les bougies / le service de scoring. Écart restant (RM-20) — voir [06](06_ecarts_doc_code.md).

### 2.7.1 SignalOutcome (issue réalisée ex post)
*Résultat a posteriori d'un signal persisté (RM-29). **Implémenté** : table dédiée `SignalOutcome`, peuplée par `SignalOutcomeEvaluationJob` (batch quotidien 03:00 UTC), enum `SignalOutcomeEnum` ([02](02_glossaire_et_taxonomies.md#signaloutcome-issue-r%C3%A9alis%C3%A9e-ex-post)).*

| Champ | Statut | Type | Sens |
|---|---|---|---|
| `analysisRunId` | Mandatory | string | Snapshot source. |
| `patternAssessmentId` | Mandatory | string | Évaluation de pattern source (unique). |
| `decisionSignalId` | Mandatory | string | Signal évalué. |
| `outcome` | Mandatory | enum | `TargetHit` / `InvalidationHit` / `StillOpen` / `TargetMiss` / `NotEvaluable` ([02](02_glossaire_et_taxonomies.md#signaloutcome-issue-r%C3%A9alis%C3%A9e-ex-post)). |
| `evaluationWindowDays` | Mandatory | int | Fenêtre appliquée = `DecisionSignal.HorizonDays` (défaut **30**). |
...
| `confidenceLabel` | Mandatory | enum | `ConfidenceLabel` du signal au moment de l'évaluation (calibration). |

**Invariants** : déterministe ; n'évalue que les signaux `IsActionable` ; compare les bougies journalières (`PriceHistory`, `1d`) aux niveaux `TargetPrice` / `InvalidationPrice` **persistés au snapshot** (jamais recalculés à partir de l'état courant, RM-20) ; émet une alerte `LevelCrossed` à la transition ouvert→terminal (RM-25) ; alimente les KPI qualité des signaux (§3.bis) et les stats win-rate Wilson (`GET patterns/statistics`).
...
| Détail paramètre (`C.7`) côté user | `LIVRÉ` | Route `parameters/:parameterId` + `GET parameters/{analysisId}/{parameterId}`. |
| Comparaison de snapshots côté user (`C.10`) | `LIVRÉ` | Route `history/compare` + `POST snapshots/compare`. |
| Learn (`C.11`) | `LIVRÉ` | Route `learn` + `GET learn`. |
| Help center (`C.13`) | `LIVRÉ` | Route `help` + surfaces `faq` / `legal-cards` / `glossary`. |
| Notifications (lecture/maj statut) | `LIVRÉ` | Entité `UserNotification` + `GET/POST Notifications` (list / mark as read). |
| Lecture support (composite + PEA) dans le payload d'analyse | `PARTIEL` | Exposée **en direct** via `fundamentals/score` + `instruments/{symbol}/fundamentals` (panneau fondamentaux) ; **non persistée au snapshot** (RM-20, écart restant). |
| Confiance expliquée (`ConfidenceBreakdown`) dans le payload | `LIVRÉ` | Assemblée par `ConfidenceBreakdownAssembler` (3 critères detection/validation/invalidation), rendue en inline dans le détail d'analyse (§2.6.1). |
| Plan d'action (`ActionPlan`) | `LIVRÉ` | `ActionPlanGenerationService` (max 3 étapes déterministes) exposé au payload + rendu inline. |
| Alertes proactives (génération + déclencheur) | `PARTIEL` | `LevelCrossed` émise par le job ex post ; préférences `alert-preferences`. **Pas d'écran d'alertes dédié** (reste). |
| Évaluation ex post (`SignalOutcome`) | `LIVRÉ` | Table `SignalOutcome` + `SignalOutcomeEvaluationJob` quotidien (§2.7.1). |
| KPI admin (4 familles) | `LIVRÉ` | `GET kpi/engagement` + `kpi/signal-quality` (+ exports), écrans admin `kpi/*`. |
| Détail paramètre user / glossaire inline | `PARTIEL` | Détail paramètre + page glossaire livrés ; **composant glossaire inline (info-bulles)** non construit (reste). |
