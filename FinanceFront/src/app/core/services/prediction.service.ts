...
export class PredictionService {
  ...
  predict(symbol: string): Observable<PredictionResult> {
    ...
    return this.http
      .get<Record<string, unknown>>(`/api/trading/predict/${encodeURIComponent(normalizedSymbol)}`)
  ...
  private normalizePrediction(payload: Record<string, unknown>, fallbackSymbol: string): PredictionResult {
    ...
    const probabilityPct =
      this.readNumber(payload, ['probabilityPct', 'ProbabilityPct']) ?? lastProbability * 100;
    const nWindows = Math.max(
      0,
      Math.round(this.readNumber(payload, ['nWindows', 'NWindows', 'n_windows']) ?? 0)
    );
    ...
    const actionReason =
      ...
      `Signal ${pattern} avec une probabilite de ${lastProbability * 100}%`;
  ...
  private readPatterns(
    ...
    if (!Array.isArray(rawPatterns)) {
      return [{ pattern: defaultPattern, probability: defaultProbability }];
    }
    ...
    return parsed.length > 0 ? parsed : [{ pattern: defaultPattern, probability: defaultProbability }];
  }
