...
export class WatchlistPageComponent {
  ...
  onSearchChanged(query: string): void {
    ...
    this.clientFinanceService.searchAssets(query, this.peaEligibleOnly).pipe(finalize(() => (this.searchLoading = false)), takeUntilDestroyed(this.destroyRef)).subscribe({ next: (results) => (this.searchResults = results), error: () => { this.searchResults = []; this.toastService.error('Recherche impossible.'); } });
  }
  ...
  addSelectedAssetToWatchlist(): void {
    ...
    this.clientFinanceService.addToWatchlist(this.selectedAsset.Symbol, this.selectedAsset.CompanyName, this.selectedAsset.Market).pipe(finalize(() => (this.watchlistLoading = false)), takeUntilDestroyed(this.destroyRef)).subscribe({ next: () => { this.toastService.success('Valeur ajoutée à la watchlist.'); this.loadWatchlist(); }, error: () => this.toastService.error('Ajout à la watchlist impossible.') });
  }
  fetchQuote(symbol: string): void {
    if (!symbol) return;
    this.quoteLoading = true;
    this.clientFinanceService.getLiveQuote(symbol).pipe(finalize(() => (this.quoteLoading = false)), takeUntilDestroyed(this.destroyRef)).subscribe({ next: (quote) => (this.selectedQuote = quote), error: () => { this.selectedQuote = null; this.toastService.error('Lecture du cours live impossible.'); } });
  }
  ...
  removeFromWatchlist(symbol: string): void {
    ...
    this.watchlistLoading = true;
    this.clientFinanceService.removeFromWatchlist(symbol).pipe(finalize(() => (this.watchlistLoading = false)), takeUntilDestroyed(this.destroyRef)).subscribe({ next: () => { this.toastService.success('Valeur retirée de la watchlist.'); this.watchlist = this.watchlist.filter((item) => item.Symbol !== symbol); }, error: () => this.toastService.error('Suppression impossible.') });
  }
  private loadWatchlist(): void {
    this.watchlistLoading = true;
    this.clientFinanceService.getWatchlist().pipe(finalize(() => (this.watchlistLoading = false)), takeUntilDestroyed(this.destroyRef)).subscribe({ next: (items) => (this.watchlist = items), error: () => { this.watchlist = []; this.toastService.error('Chargement de la watchlist impossible.'); } });
  }
}
